...
_BEFORE
	...
	account[14]=0;
#ifdef _DEBUG
	log_1.AddLog( LOG_WARN, "Call StartIPChargeFail,%s,%d,%d.%d.%d.%d", account, ErrorCode, ip.S_un.S_un_b.s_b1, ip.S_un.S_un_b.s_b2, ip.S_un.S_un_b.s_b3,ip.S_un.S_un_b.s_b4);	
#endif
	if ( uid > 0 )
		accountdb.logoutAccount( uid );
_AFTER_FIN	
...
_BEFORE
#ifdef _DEBUG
	log_1.AddLog( LOG_WARN, "RCV: IA_IP_START_OK," );
#endif
...
_BEFORE
#ifdef _DEBUG
	log_1.AddLog( LOG_WARN, "RCV: IA_IP_READY_OK," );
#endif
	int Uid=0, SpecTime=0, kind=0, ip=0;
	Uid = GetIntFromPacket( packet );
	unsigned char tempWorldID = GetCharFromPacket( packet );
