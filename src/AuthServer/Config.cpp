	...
	if(pBuf) {
		...
		while ((token = GetToken(inp, value, true)) != eEOF) {
			if (state == eInComment) {
				...
				case eWord:
					...
					case eWaitValue:
						...
						if (result.second) {
							...
							log_1.AddLog(LOG_ERROR, "Insert twice %s at line %d in %s", key, line, filename);
						}
						break;
					case eInComment:
						break;
					default:
						log_1.AddLog(LOG_ERROR, "Invalid state at line %d in %s", line, 
                          filename);
					}
					break;
				case eEqual:
				...
				default:
					if (state != eInComment) {
						log_1.AddLog(LOG_ERROR, "Invalid token %s at line %d in %s", value, 
                          line, filename);
					}
					break;
				}
		...
		log_1.AddLog(LOG_ERROR, "Can't load %s", filename);
