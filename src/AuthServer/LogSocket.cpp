...
{
/*
	log_1.AddLog( LOG_WARN, "Timer Callback Reconnect LOGDTimer Timer Called" );
_BEFORE
	if ( g_bTerminating )
	{
		ReleaseRef();
	...
	if (m_hSocket == INVALID_SOCKET) {
		log_1.AddLog(LOG_ERROR, "log socket error %d", WSAGetLastError());
	...
	if ( ErrorCode == SOCKET_ERROR ){
		...
		ReleaseRef();
		log_1.AddLog( LOG_WARN, "LOGD Socket Connected" );
		return ;
	}
_AFTER_FIN
*/
...
void CLogSocket::OnClose(SOCKET closedSocket)
{
	...
	log_1.AddLog(LOG_ERROR, "*close logd connection from %s, %x(%x)", IP(), closedSocket, this);
