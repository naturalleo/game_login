...
static bool LoginPacketSecure(CSocketServerEx *mysocket, const unsigned char *packet, bool useMD5)
{
	...
	if ( config.DesApply ){
		...
		{
			log_1.AddLog( LOG_ERROR, "SND: AC_LOGIN_FAIL,invalid_key_size" );
    ...
    log_1.AddLog( LOG_VERBOSE, "RCV: AQ_LOGIN,account:%s,ip:%d.%d.%d.%d,subscription:%d,cdkind:%d", name, cip.S_un.S_un_b.s_b1, cip.S_un.S_un_b.s_b2, cip.S_un.S_un_b.s_b3, cip.S_un.S_un_b.s_b4, subscription, cdkind);
	log_1.AddLog( LOG_DEBUG, "RCV: AQ_LOGIN,useMD5:%i, MD5Key %i", useMD5, mysocket->GetMd5Key());
	...
	if ( err_msg != S_ALL_OK )
	{
		...
		if ( config.Country == CC_KOREA ) {
			log_1.AddLog( LOG_ERROR, "계정 %s 암호 체크에 실패 했습니다.  MSG: %d", name, err_msg );
		} else
			log_1.AddLog( LOG_ERROR, "SND: AC_LOGIN_FAIL,Error Msg: %d", err_msg );
#endif
...
_BEFORE
	...
	if ( pKey != md5key ){
		pSocket->Send( "cc", AC_PLAY_FAIL, S_NO_LOGININFO );		
		log_1.AddLog( LOG_ERROR, "md5key does not matching" );
		return false;
	}
...
{
	log_1.AddLog( LOG_ERROR, "Received a deprecated packet from client connection.");
	return true;
}
...
//decode routine need
/////////////////////////////
					log_1.AddLog( LOG_DEBUG, "Received %d bytes DecKey 0x87546CA1, 0x%x", packetLen+2, DecOneTimeKey);
					pi += 2;
					mode = SM_READ_BODY;
				}
			} else {
			...
			if (pi + packetLen <= ri) {
				...
				if ( !result ){
#ifdef _DEBUG
					log_1.AddLog( LOG_ERROR, "unknown protocol,decrypt error" );
#endif
					break;
				}
#ifdef _DEBUG
...
void CSocketServerEx::Send( const char *sendmsg, int msglen )
{
	...
	log_1.AddLog( LOG_DEBUG, "Sent %d bytes, EncKey 0x87546CA1, 0x%x", msglen, EncOneTimeKey);
#ifdef _DEBUG
