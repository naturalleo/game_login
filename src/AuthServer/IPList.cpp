...
BOOL CIPList::Load(char *fileName)
{
	...
	for ( ; ; ) {
		...
		if (record.begin >= record.end) {
			log_1.AddLog(LOG_ERROR, "IPList: start address address is greater than end address at line %d", line);
			continue;
		}
		...
		if (it != table.end() && it->begin <= record.begin-1) {
			if (it->end != record.begin)
				log_1.AddLog(LOG_ERROR, "IPList: address is merged with %d.%d.%d.%d-%d.%d.%d.%d at line %d",
	...
	lock.WriteUnlock();
	log_1.AddLog(LOG_NORMAL, "IPList loaded from %s", fileName);
	return TRUE;
}
