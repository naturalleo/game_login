...
unsigned __stdcall TimerThread(void *)
{
	...
	job.RunTimer();
	log_1.AddLog( LOG_DEBUG, "Timer thread terminated");
...
_BEFORE
	...
	for ( ; ; ) {
		...
		if (g_bTerminating) {
			log_1.AddLog( LOG_DEBUG, "terminate IOThreadServer");
			break;
		}
	...
	log_1.AddLog( LOG_DEBUG, "IOThread Server Exit");
_AFTER_FIN
	return 0;
}
...
_BEFORE
	...
	for ( ; ; ) {
		...
		if (g_bTerminating) {
			log_1.AddLog( LOG_DEBUG, "terminate IOThreadInt");
			break;
		}
